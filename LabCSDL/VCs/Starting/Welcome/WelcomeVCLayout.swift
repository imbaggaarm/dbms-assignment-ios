...
class WelcomeVCLayout: BaseViewControllerLayout, UITextViewDelegate {
    ...
    static let strTitle = "Cùng học với chuyên gia để tiến bộ nhanh hơn"
...
//        temp.image = AppIcon.appIcon
    ...
    func setUpTxtVLogin() {
        ...
        let string = NSMutableAttributedString.init(string: WelcomeVCLayout.strAccount, attributes: [NSAttributedString.Key.font: UIFont.avenirNext(size: 16, type: .regular), NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.paragraphStyle: paragraph])
