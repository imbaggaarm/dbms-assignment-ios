...
class IMBInputMessageBar: UIView {
    ...
    let whiteBackgroundView: UIView = {
        let temp = UIView()
        temp.backgroundColor = UIColor.init(white: 1, alpha: 1)
        return temp
    }()
...
class IMBInputContentView: UIView, UITextViewDelegate {
    ...
    lazy var txtVInputMessage: IMBTextView = {
        ...
        temp.placeHolderFont = UIFont.systemFont(ofSize: 16)
        temp.placeHolderColor = .gray
        temp.normalFont = UIFont.systemFont(ofSize: 16)
        temp.normalTextColor = UIColor.black
        temp.backgroundColor = .clear
        temp.placeHolder = "Aa"
        temp.setPlaceHolder()
        temp.delegate = self
